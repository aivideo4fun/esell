...
export default function AdminLayout({
  ...
  // Login page par sidebar render nahi hoga
  if (isLoginPage) {
    return <>{children}</>;
  }
      ...
      <aside className="w-full md:w-64 bg-gray-950 text-white p-6 flex flex-col justify-between shrink-0">
        <div className="space-y-8">
          ...
          <nav className="space-y-2">
            ...
            >
              <ShoppingCart className="w-4 h-4 text-emerald-400" /> Orders &amp; Fulfillment
            </Link>

            {/* Inventory & Stock Manager Tab */}
            <Link
            ...
            >
              <Layers className="w-4 h-4 text-amber-400" /> Stock &amp; Inventory
            </Link>
            ...
            >
              <Package className="w-4 h-4 text-purple-400" /> Products Catalog
            </Link>
          </nav>
        </div>
