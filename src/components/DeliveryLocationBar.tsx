...
import { MapPin, Navigation, ChevronDown, X, Loader2, Truck } from "lucide-react";
...
export default function DeliveryLocationBar() {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          ...
          if (data.success && data.pincode) {
            ...
            setErrorMsg("Could not detect exact location. Please enter pincode manually.");
          }
        } catch {
          setErrorMsg("Failed to resolve GPS location.");
        } finally {
          setGpsLoading(false);
        }
      },
      (err) => {
        ...
        if (err.code === err.PERMISSION_DENIED) {
          setErrorMsg("Location permission denied. Please allow GPS or enter pincode.");
        } else {
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success && data.city) {
        ...
        setErrorMsg("Pincode not found. Please check and retry.");
      }
    } catch {
  ...
  return (
    <>
      {/* Top Strip */}
      <div className="bg-[#f0fdf4] border-b border-emerald-100/80 px-4 sm:px-6 lg:px-8 py-1.5 text-xs text-slate-700">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          {/* Delivering to with City underneath */}
          <button
            type="button"
          ...
          >
            ...
            <div className="flex flex-col leading-tight">
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs font-black text-slate-900 font-mono tracking-tight underline decoration-dotted">
                  {pincode}
                </span>
                <ChevronDown className="w-3 h-3 text-slate-400 group-hover:text-slate-700 transition" />
              </div>
              {/* Pincode ke theek niche city */}
              <span className="text-[10px] font-bold text-emerald-700 line-clamp-1 max-w-[140px]">
                {city}
              </span>
            </div>
          </button>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-slate-500 font-medium">
                  Delivery options and speed depend on your exact location.
                </p>
              </div>
              <button
